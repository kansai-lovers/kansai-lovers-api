class Member < ApplicationRecord
  ...
  def dev_design()
    ...
    # stdが0になって0除算
    return ((self.total_hooray_count - avg) * 10 / std + 50).round(2)
  end
  ...
  def dev_implementation()
    ...
    std = Math.sqrt(arr1.sum / arr_x.length)
    # stdが0になって0除算
    return ((self.total_rocket_count - avg) * 10 / std + 50).round(2)
  end
  ...
  def dev_communication()
    ...
    std = Math.sqrt(arr1.sum / arr_x.length)
    # stdが0になって0除算
    return ((self.total_heart_count - avg) * 10 / std + 50).round(2)
  end
end
